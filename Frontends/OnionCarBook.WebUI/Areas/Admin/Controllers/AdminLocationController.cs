    ...
    public class AdminLocationController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            //{
                ...
                var responseMessage = await client.GetAsync("https://localhost:7077/api/Locations");
        ...
        public async Task<IActionResult> CreateLocation(CreateLocationDto createLocationDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7077/api/Locations", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7077/api/Locations?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminLocation", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7077/api/Locations/{id}");
        ...
        public async Task<IActionResult> UpdateLocation(UpdateLocationDto updateLocationDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7077/api/Locations/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminLocation", new { area = "Admin" });
            }
